...
function ProjectAdminSettings(props) {
    ...
    useEffect(() => {
        axios.get(`http://localhost:8080/project/getProjectById/${props.id}`)
            .then(response => {
    ...
    async function handleProjectEdit(e){
        ...
        await axios.post(`http://localhost:8080/project/editProject/${props.id}`, bodyFormData, {headers:{
                'Authorization': Cookies.get("authorization")
            }})
            .catch(err => {
                console.log(err.response)
