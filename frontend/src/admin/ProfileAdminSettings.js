...
function ProfileAdminSettings(props) {
    ...
    useEffect(() => {
        ...
        axios.get(`http://localhost:8080/profile?id=${props.id}`)
            .then(response => {
    ...
    async function handleProfileEdit(e){
        ...
        await axios.post(`http://localhost:8080/profile?userId=${props.id}`, bodyFormData, {headers:{
                'Authorization': Cookies.get("authorization")
            }}).then(res => {
...
export default ProfileAdminSettings;